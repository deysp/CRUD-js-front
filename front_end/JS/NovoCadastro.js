...
document
  ...
  .addEventListener("click", async (event) => {
    ...
    if (!senhaRegex.test(senha)) {
      alert(
        "A senha deve ter 8 caracteres, incluindo maiúsculas, minúsculas, números e símbolos."
      );
      return;
    }
...
document
  ...
  .addEventListener("click", async (event) => {
    ...
    const senhaConfirmacao = document
      .querySelector("#senhaConfirmacao")
      .value.trim();
